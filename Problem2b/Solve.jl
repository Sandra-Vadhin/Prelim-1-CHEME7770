...
function calculate_discrete_steady_state(ic_array, data_dictionary)
    ...
    while (is_ok_to_stop == false)
        ...
        (T1,soln_array_1,param_array_1) = GRNDiscreteDynamicSolve((time_start, time_stop, time_step_size), data_dictionary)

        # run for one more step -
        data_dictionary[:initial_condition_array] = soln_array_1[end,:]
        (T2,soln_array_2,param_array_2) = GRNDiscreteDynamicSolve((T1[end], T1[end]+1.0, time_step_size), data_dictionary)

        # compute the differnce -
        residual = soln_array_1[end,:] - soln_array_2[end,:]
        residual2 = param_array_1[end,:] - param_array_2[end,:]
...
function GRNDiscreteDynamicSolve(time_span::Tuple{Float64, Float64, Float64}, data_dictionary::Dict{Symbol,Any})
    ...
    init_kinetics_array = calculate_txtl_kinetics_array(0,initial_condition_array,data_dictionary)
    number_of_kinetic_parameters = length(init_kinetics_array)

    init_control_array = calculate_control_array(0,initial_condition_array,data_dictionary)
    init_rV = init_kinetics_array.*init_control_array

    # initalize -
    X = zeros(number_of_time_steps, number_of_states)
    Z = zeros(number_of_time_steps,number_of_kinetic_parameters)
    # put the IC into the X array -
    for index = 1:number_of_states
        X[1,index] = initial_condition_array[index]
    end

    for j = 1:number_of_kinetic_parameters
        Z[1,j] = init_rV[j]
    end
    ...
    for time_value_index = 1:number_of_time_steps - 1
        ...
        (next_state_array,rV) = discrete_dynamic_balances(current_time_value, current_state_array, data_dictionary)
        ...
        for j = 1:length(rV)
            Z[time_value_index+1,j] = rV[j]
        end

    end

    # return -
    return (simulation_time_array, X,Z)
end
